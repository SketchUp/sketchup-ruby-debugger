...
{
public:
    Connection(boost::asio::io_service& service, int port, IDebugServer* server, boost::condition_variable& serverWaitCond, boost::mutex& serverWaitMutex, bool& serverCanContinue);
...
RDIP::Connection::Connection(boost::asio::io_service& service, int port, IDebugServer* server, boost::condition_variable& serverWaitCond, boost::mutex& serverWaitMutex, bool& serverCanContinue)
    : mSocket(service)
...
void RDIP::Connection::evaluateCommand(const std::string& cmd)
{
    ...
    { 
        if(what.size() == 2)
        {
            ...
            size_t activeFrameIndex = mServer->GetActiveFrameIndex();
            bool shift_up = true;
            size_t delta = 0;
            if(activeFrameIndex > frameIndex)
            {
                shift_up = false;
                delta = activeFrameIndex - frameIndex;
            }
            for(size_t i = 0; i < delta; ++i)
            {
                mServer->ShiftActiveFrame(shift_up);
            }
        }
    }
    else if(regex_match(cmd, what, reg_step))
    {
        OutputDebugStringA("reg_step\n");
    }
    ...
    {
        OutputDebugStringA("reg_up\n");
    }
    else if(regex_match(cmd, what, reg_eval))
    {
    ...
    {
        auto variables = mServer->GetGlobalVariables();

        write(mSocket, boost::asio::buffer("<variables>\n"));
        for(const auto var : variables)
        {
            boost::format fmt("<variable name=\"%1%\" kind=\"global\" value=\"%2%\" type=\"Unknown\" hasChildren=\"false\" objectId=\"0xdead0000\"/>");
            fmt % var.first % var.second;
            write(mSocket, boost::asio::buffer(fmt.str()));
        }
        write(mSocket, boost::asio::buffer("</variables>\n"));
